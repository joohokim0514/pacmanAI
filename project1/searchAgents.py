...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        return len(cornersVisited) is len(self.corners)
            ...
            if not hitsWall:
                ...
                if nextPosition not in cornersVisited:
                    if nextPosition in self.corners:
                        successors.append(((nextPosition, cornersVisited + [nextPosition]), action, 1))
                    else:
...
def cornersHeuristic(state, problem):
    ...
    while len(cornersUnvisited)>0:
        ...
        cornersUnvisited.remove(closestCorner)
    return h
...
def foodHeuristic(state, problem):
    ...
    h = myMazeDistance(position, maxFood, problem.getWalls())
    return h

def myMazeDistance(position, food, walls):
    visited = []
    path = []
    q = util.Queue()
    goal = food
    q.push((position, path))
    while not q.isEmpty():
        state, path = q.pop()
        if state not in visited:
            ...
            for successor in myMazeSuccessors(state, walls):
                nextState, action, cost = successor
                if nextState not in visited:
                    newAction = [action]
                    q.push((nextState, path + newAction))

    return len(path)

def myMazeSuccessors(state, walls):
    successors = []
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
        x, y = state
        dx, dy = Actions.directionToVector(action)
